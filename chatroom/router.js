...
router.get('/chatrooms', (req, res, next) => {
  console.log('got a request on /chatroom with userId', req.query.userId);

  db.query(`SELECT chatrooms.* FROM chatrooms INNER JOIN members ON chatrooms.id = members."chatroomId" WHERE members."userId" = ${req.query.userId}`)
